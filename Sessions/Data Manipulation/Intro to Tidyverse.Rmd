...
The Sea Around Us (www.seaaroundus.org) is a valuable source of information related to fisheries, from catch data, to economics and more! Fortunatly there's a *R* package to access the data, you can find the documentation [here](https://cran.r-project.org/web/packages/seaaroundus/seaaroundus.pdf).
...
unique(Tidy_Data$nation)
unique(Tidy_Data$years)
...
arrange(Data, 
        desc(years) # use desc() for decreasing
        )
...
# Convert years to charactes so we can filter
Spain_Data$Example <- as.character(Data$years)
...
filter(Data,
       Example %in% Selection,
       portugal > 0)
# This will give me the selected years but only those years that Spain fished in Portugal waters
...
filter_starts_with_c <- filter(Data, str_detect(common_name, pattern="2"))
...
filter_salmon <- filter(Data, str_detect(common_name, pattern="salmon|crab" ))
